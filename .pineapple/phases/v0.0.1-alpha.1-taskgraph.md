...
> **Status: awaiting human approval — do not dispatch until approved.**
> After approval, run `/pineapple:orchestrate v0.0.1-alpha.1`.
...
| Aspect | Contract |
|---|---|
| Base URLs / env | `apiBaseUrl`, `realtimeWsUrl`, `webAdminBaseUrl` via `--dart-define`; `useMocks` via `bool.fromEnvironment('USE_MOCKS', defaultValue: true)` |
...
| Wire-format | snake_case on the wire → camelCase `freezed` via `@JsonKey`; `exp`/`iat` Unix-seconds **numbers**; `tenant_id` optional/absent (`INV-CLIENT-AUTH-3`); signup `organization_id` present-as-null |
...
| **P1-T-03** | auth core: `AuthRepository` (+mock), `AuthState` union, `AuthNotifier` state machine, session restore, token-holder writes | `auth.md` | `features/auth` | frontend | high | P1-T-01 | auto |
...
- `appConfigProvider` exposes `apiBaseUrl`/`realtimeWsUrl`/`webAdminBaseUrl`/`useMocks` from `--dart-define`; `useMocks` defaults `true`.
...
- `badCertificateCallback` returns `true` only on SHA-256 pin match (rotation list) (`INV-CLIENT-NET-1`); a dev escape hatch is gated to debug/`useMocks` and never ships enabled in release.
...
- `AuthRepository` maps snake_case → camelCase `AuthSession`; coercions applied (`exp` seconds, `tenant_id` optional); a `MockAuthRepository` backs `useMocks=true`.
- `build()` restores session from secure storage on cold start.
- *Verify:* `flutter analyze` + unit tests over every transition + token-holder writes + mock path.
...
- **No `human_admin` tasks.** Real staging base URLs + cert-pin SHA-256 values are needed to run against live `leo-api`, but with `useMocks=true` (default) every task builds, tests, and demos without them — supply real values at integration time.
