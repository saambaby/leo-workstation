...
**Scope (files):**
- `data/auth_repository.dart` — `ApiAuthRepository.login` gains optional `totpCode`; returns a result distinguishing `enrollmentRequired` (carries `otpauthUrl`/`secret`/`enrollmentToken`) from `mfaRequired` (flag only); `mfa/enroll` call added; **delete** `selectMembership`/`switchTenant`/`listMemberships`; `acceptInvite` sends nested `consent {tos, privacy, baa_ack}`. `MockAuthRepository` updated to the same shape (drop `multi@` picker case; drop `/auth/mfa` path; resubmit-login MFA).
- `domain/auth_models.dart` — **delete** `LoginResult.pickMembership` arm; enrollment payload on the MFA-required result.
