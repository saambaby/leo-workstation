...
- **LSP native signup scheduling (open founder call):** [`features/lsp-native-signup.md`](features/lsp-native-signup.md) is specced, cross-spec-audit-clean, and its taskgraph ([`v0.0.1-p2-onboarding-lsp-taskgraph.md`](phases/v0.0.1-p2-onboarding-lsp-taskgraph.md)) is Gate-1 approved — but not yet orchestrated. Still needs a decision on fold-in vs. separate-pass timing relative to `v0.0.1-p2-onboarding`; the taskgraph's `owns` is disjoint from that phase's tasks either way, so approval didn't force the answer.
