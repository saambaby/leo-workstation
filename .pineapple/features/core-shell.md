...
2. `appConfigProvider` exposes `apiBaseUrl`, `realtimeWsUrl`, and `webAdminBaseUrl?` from compile-time env (`--dart-define`). No mock/live toggle — the client always talks to the real `leo-api` at `apiBaseUrl` (retired 2026-07-01; see `docs/release-plan.md` § Mock strategy).
...
- **`AppConfig`** is sourced at compile time: `apiBaseUrl`/`realtimeWsUrl`/`webAdminBaseUrl` via `String.fromEnvironment`. No runtime mutation; it's a `Provider<AppConfig>` of an immutable struct. **`webAdminBaseUrl` is the single source** for both the `lsp_admin` "Admin dashboard" link (here) and `router`'s `platform_admin` `/web-handoff` interstitial.
...
- **Cert-pin blast radius — RESOLVED 2026-07-01:** with no pins configured, `pinMatches` allows the connection only in a debug build (`kDebugMode`) — never a release bypass. No mock-mode escape hatch exists since there's no mock mode.
